<?php get_header(); ?>

<section class="content-fluid min-height main-cont-home">
    <div class="row">
        <div class="col-sm-7 card-project">
            <h4 data-toggle="tooltip" data-placement="top" title="Project Name">
                New Project Name
            </h4>
            <a class="author-proj" href="#">
                <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20 8.57141C18.5873 8.57141 17.2063 8.99033 16.0317 9.7752C14.857 10.5601 13.9415 11.6756 13.4009 12.9808C12.8603 14.286 12.7188 15.7222 12.9944 17.1078C13.27 18.4933 13.9503 19.7661 14.9493 20.765C15.9482 21.764 17.221 22.4443 18.6065 22.7199C19.9921 22.9955 21.4283 22.854 22.7335 22.3134C24.0387 21.7728 25.1542 20.8573 25.9391 19.6826C26.724 18.508 27.1429 17.127 27.1429 15.7143C27.1429 13.8199 26.3903 12.0031 25.0508 10.6635C23.7113 9.32396 21.8944 8.57141 20 8.57141Z" fill="currentColor"/>
                    <path d="M20 0C16.0444 0 12.1776 1.17298 8.8886 3.37061C5.59962 5.56823 3.03617 8.69181 1.52242 12.3463C0.00866566 16.0009 -0.387401 20.0222 0.384303 23.9018C1.15601 27.7814 3.06082 31.3451 5.85787 34.1421C8.65492 36.9392 12.2186 38.844 16.0982 39.6157C19.9778 40.3874 23.9991 39.9913 27.6537 38.4776C31.3082 36.9638 34.4318 34.4004 36.6294 31.1114C38.827 27.8224 40 23.9556 40 20C39.9939 14.6975 37.8849 9.61396 34.1355 5.86454C30.386 2.11513 25.3025 0.0060496 20 0ZM31.4171 32.7514C31.3887 30.8776 30.6252 29.09 29.2913 27.7737C27.9574 26.4574 26.1597 25.7178 24.2857 25.7143H15.7143C13.8403 25.7178 12.0426 26.4574 10.7087 27.7737C9.37478 29.09 8.61133 30.8776 8.58286 32.7514C5.99224 30.4382 4.16534 27.3927 3.34406 24.0181C2.52278 20.6435 2.74586 17.0991 3.98378 13.8541C5.22169 10.6091 7.41604 7.81667 10.2763 5.84652C13.1365 3.87638 16.5276 2.82146 20.0007 2.82146C23.4738 2.82146 26.865 3.87638 29.7252 5.84652C32.5854 7.81667 34.7797 10.6091 36.0177 13.8541C37.2556 17.0991 37.4787 20.6435 36.6574 24.0181C35.8361 27.3927 34.0092 30.4382 31.4186 32.7514H31.4171Z" fill="currentColor"/>
                </svg>
                <p>Username</p>
            </a>
            <p class="proj-description">Lorem ipsum dolor sit amet vulputate in eum accumsan nulla ea sed ipsum gubergren ipsum et ea no sanctus lorem et sed tempor stet lorem duo est ut gubergren duo et ut accusam invidunt ipsum sadipscing sanctus nulla kasd eu elit eirmod sadipscing amet vero vero eirmod at amet dolor liber elitr stet kasd at esse dolor ea dolor ut nisl accusam consequat sea feugait eum vero dolores sed voluptua sed dolore et sea ut amet labore dolore labore magna erat dolore dolores wisi facilisis vel dolore eirmod gubergren amet facilisis ut sanctus duis dolor at nostrud labore consequat sed
            </p>
            <div class="container-photos">
                <div class="prime-img">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                </div>
                <div class="sec-imgs">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                </div>
            </div>
        </div>

        <div class="col-sm-7 card-project">
        <h4 data-toggle="tooltip" data-placement="top" title="Project Name">
                New Project Name
            </h4>
            <div class="author-proj">
                <svg viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20 8.57141C18.5873 8.57141 17.2063 8.99033 16.0317 9.7752C14.857 10.5601 13.9415 11.6756 13.4009 12.9808C12.8603 14.286 12.7188 15.7222 12.9944 17.1078C13.27 18.4933 13.9503 19.7661 14.9493 20.765C15.9482 21.764 17.221 22.4443 18.6065 22.7199C19.9921 22.9955 21.4283 22.854 22.7335 22.3134C24.0387 21.7728 25.1542 20.8573 25.9391 19.6826C26.724 18.508 27.1429 17.127 27.1429 15.7143C27.1429 13.8199 26.3903 12.0031 25.0508 10.6635C23.7113 9.32396 21.8944 8.57141 20 8.57141Z" fill="currentColor"/>
                    <path d="M20 0C16.0444 0 12.1776 1.17298 8.8886 3.37061C5.59962 5.56823 3.03617 8.69181 1.52242 12.3463C0.00866566 16.0009 -0.387401 20.0222 0.384303 23.9018C1.15601 27.7814 3.06082 31.3451 5.85787 34.1421C8.65492 36.9392 12.2186 38.844 16.0982 39.6157C19.9778 40.3874 23.9991 39.9913 27.6537 38.4776C31.3082 36.9638 34.4318 34.4004 36.6294 31.1114C38.827 27.8224 40 23.9556 40 20C39.9939 14.6975 37.8849 9.61396 34.1355 5.86454C30.386 2.11513 25.3025 0.0060496 20 0ZM31.4171 32.7514C31.3887 30.8776 30.6252 29.09 29.2913 27.7737C27.9574 26.4574 26.1597 25.7178 24.2857 25.7143H15.7143C13.8403 25.7178 12.0426 26.4574 10.7087 27.7737C9.37478 29.09 8.61133 30.8776 8.58286 32.7514C5.99224 30.4382 4.16534 27.3927 3.34406 24.0181C2.52278 20.6435 2.74586 17.0991 3.98378 13.8541C5.22169 10.6091 7.41604 7.81667 10.2763 5.84652C13.1365 3.87638 16.5276 2.82146 20.0007 2.82146C23.4738 2.82146 26.865 3.87638 29.7252 5.84652C32.5854 7.81667 34.7797 10.6091 36.0177 13.8541C37.2556 17.0991 37.4787 20.6435 36.6574 24.0181C35.8361 27.3927 34.0092 30.4382 31.4186 32.7514H31.4171Z" fill="currentColor"/>
                </svg>
                <p>Username</p>
            </div>
            <p class="proj-description">Lorem ipsum dolor sit amet vulputate in eum accumsan nulla ea sed ipsum gubergren ipsum et ea no sanctus lorem et sed tempor stet lorem duo est ut gubergren duo et ut accusam invidunt ipsum sadipscing sanctus nulla kasd eu elit eirmod sadipscing amet vero vero eirmod at amet dolor liber elitr stet kasd at esse dolor ea dolor ut nisl accusam consequat sea feugait eum vero dolores sed voluptua sed dolore et sea ut amet labore dolore labore magna erat dolore dolores wisi facilisis vel dolore eirmod gubergren amet facilisis ut sanctus duis dolor at nostrud labore consequat sed
            </p>
            <div class="container-photos">
                <div class="prime-img">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                </div>
                <div class="sec-imgs">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                    <img src="<?php echo get_theme_file_uri( 'assets/default-img.svg' ); ?>" alt="default image">
                </div>
            </div>
        </div>
    </div>
</section>

<?php get_footer(); ?> 